/*****************************************************************************/
/* Binary data to Z80 Disassembler class                                     */
/*                                                                           */
...
namespace DominoResourceFileViewer
{
  ...
  /// </summary>
  public class dispZ80Disassembler : IBinaryViewer
  {
    #region · Data members ·
    ...
    public string ConvertToRTF(byte[] in_buffer)
    {
      ...
      while (m_pc < m_buffer.Length)
      {
        ...
        prev_pc = m_pc;
        disassembly_buffer = "{\\b " + DisassembleOneInstruction() + "}";

        // add instruction opcodes
        line_buffer += "{\\cf3 ";
        while (prev_pc < m_pc)
        {
          line_buffer += m_buffer[prev_pc++].ToString("X2") + " ";
        }
    ...
    public string ConvertToHTML(byte[] in_buffer)
    {
      ...
      while (m_pc < m_buffer.Length)
      {
        ...
        // disassemble instruction
        prev_pc = m_pc;
        disassembly_buffer = "<b>" + DisassembleOneInstruction() + "</b>";

        // instruction opcodes
        opcode_string = "";
        while (prev_pc < m_pc)
        {
          opcode_string += m_buffer[prev_pc++].ToString("X2") + " ";
        }
    ...
    /// <summary>
    /// Gets next opcode from the buffer
    /// </summary>
    /// <returns></returns>
    private byte NextOpcode()
    {
      ...
      if (m_pc < m_buffer.Length)
      {
        opcode = m_buffer[m_pc++];
      }
      ...
      return opcode;
		...
		/// </summary>
		/// <returns></returns>
    private string DisassembleOneInstruction()
    {
      ...
      switch (opcode)
      {
        ...
        case 0xDD:
          ...
          switch (opcode)
          {
            case 0xCB: format = mnemonicsDDCB[opcode]; break;
            case 0xED: format = mnemonicsED[opcode]; break;
            default: format = mnemonicsDD[opcode]; break;
          }
          break;

        case 0xED:
          opcode = NextOpcode();
          format = mnemonicsED[opcode];
          break;
        ...
        case 0xFD:
          ...
          switch (opcode)
          {
            case 0xCB: format = mnemonicsFDCB[opcode]; break;
            case 0xED: format = mnemonicsED[opcode]; break;
            default: format = mnemonicsFD[opcode]; break;
          }
          break;

        default: format = mnemonics[opcode]; break;
      }
    ...
    readonly static string[] mnemonics = new string[]
		{
			...
			"CALL NZ, nn", "PUSH BC", "ADD A, n", "RST 00h", //0xC8
			"RET Z", "RET", "JP Z, nn", "[CB]", //0xCC
			"CALL Z, nn", "CALL nn", "ADC A, n", "RST 08h", //0xD0
			"RET NC", "POP DE", "JP NC, nn", "OUT n, A", //0xD4
			"CALL NC, nn", "PUSH DE", "SUB n", "RST 10h", //0xD8
			"RET C", "EXX", "JP C, nn", "IN A, n", //0xDC
			"CALL C, nn", "[DD]", "SBC A, n", "RST 18h", //0xE0
			"RET PO", "POP HL", "JP PO, nn", "EX (SP), HL", //0xE4
			"CALL C, nn", "PUSH HL", "AND n", "RST 20h", //0xE8
			"RET PE", "JP HL", "JP PE, nn", "EX DE, HL", //0xEC
			"CALL PE, nn", "[ED]", "XOR n", "RST 28h", //0xF0
			"RET P", "POP AF", "JP P, nn", "DI", //0xF4
			"CALL P, nn", "PUSH AF", "OR n", "RST 30h", //0xF8
			"RET M", "LD SP, HL", "JP M, nn", "EI", //0xFC
			"CALL M, nn", "[FD]", "CP n", "RST 38h", //0x100
    };
    ...
    readonly static string[] mnemonicsDD = new string[]
		{
			...
			"CALL NC, nn", "PUSH DE", "SUB n", "RST 10h", //0xD8
			"RET C", "EXX", "JP C, nn", "IN A, n", //0xDC
			"CALL C, nn", "[!DD DD!]", "SBC A, n", "RST 18h", //0xE0
			"RET PO", "POP IX", "JP PO, nn", "EX (SP), IX", //0xE4
			"CALL C, nn", "PUSH IX", "AND n", "RST 20h", //0xE8
			"RET PE", "JP IX", "JP PE, nn", "EX DE, HL", //0xEC
			"CALL PE, nn", "[DD ED]", "XOR n", "RST 28h", //0xF0
			"RET P", "POP AF", "JP P, nn", "DI", //0xF4
			"CALL P, nn", "PUSH AF", "OR n", "RST 30h", //0xF8
			"RET M", "LD SP, IX", "JP M, nn", "EI", //0xFC
			"CALL M, nn", "[!!DD FD!!]", "CP n", "RST 38h", //0x100
		};
    ...
    readonly static string[] mnemonicsFD = new string[]
		{
			...
			"CALL NC, nn", "PUSH DE", "SUB n", "RST 10h", //0xD8
			"RET C", "EXX", "JP C, nn", "IN A, n", //0xDC
			"CALL C, nn", "[!FD DD!]", "SBC A, n", "RST 18h", //0xE0
			"RET PO", "POP IY", "JP PO, nn", "EX (SP), IY", //0xE4
			"CALL C, nn", "PUSH IY", "AND n", "RST 20h", //0xE8
			"RET PE", "JP IY", "JP PE, nn", "EX DE, HL", //0xEC
			"CALL PE, nn", "[FD ED]", "XOR n", "RST 28h", //0xF0
			"RET P", "POP AF", "JP P, nn", "DI", //0xF4
			"CALL P, nn", "PUSH AF", "OR n", "RST 30h", //0xF8
			"RET M", "LD SP, IY", "JP M, nn", "EI", //0xFC
			"CALL M, nn", "[!FD FD!]", "CP n", "RST 38h", //0x100
		};
