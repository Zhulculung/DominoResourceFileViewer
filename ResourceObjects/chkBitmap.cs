...
namespace DominoResourceFileViewer
{
	...
	{
		#region · Constants ·
		private const int BitsPerPixelMask = 0x3f;
		private const int BitmapDataOffsetShift = 6;
		#endregion
		...
		public class BitmapInfo
    {
      ...
      {
        return (Width * BitPerPixel + 7) / 8 * Height + sizeof(UInt16) + sizeof(UInt16) + sizeof(byte) + BitmapDataOffset;
      }
    }
		#endregion
		...
		public override bool Load(UInt32 in_chunk_id, byte[] in_buffer, int in_offset)
    {
      ...
      while (pos < in_buffer.Length)
      {
        ...
        // load bits per pixel
        bitmap_info.BitPerPixel = in_buffer[pos] & BitsPerPixelMask;
				bitmap_info.BitmapDataOffset = in_buffer[pos] >> BitmapDataOffsetShift;
				pos += sizeof(byte) + bitmap_info.BitmapDataOffset;
